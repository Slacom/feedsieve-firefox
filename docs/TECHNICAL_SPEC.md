...
- 指纹：64bit 确定性哈希，16 位小写 hex；归一化后短于 12 字符不产指纹
  （防无关内容碰撞）。v0.5 起指纹即 SimHash（`simhash.ts`，位向量 + 汉明距离
  ≤2 判话术变体），v0.4 的 cyrb53 精确指纹与新的 simhash 在快照中并存，
  exact 优先、变体兜底
...
指纹 v1（2026-09-07，`fingerprint.ts` / `simhash.ts` 新增 v1 入口）：

- v1 归一化：先 NFKC 折叠（全角字母/数字变半角）再走 v0 管线；零宽/控制字符
  由剥离正则消除。全角规避（`ｄｍ　ｍｅ`）不再漏
- 停用字：中英高频字组成的 n-gram 不参与投票，降低正常文本与垃圾模板的
  伪相似度（v0 的 12 字符门槛是伪相似度与覆盖度的平衡点，v1 把门槛
  语义改为「有效 n-gram 数」，短话术（≥8 字符）可产指纹
- 版本位：v1 值高 4 bit 固定为 `0x3`（恒以 `3` 开头），低 60 bit 为 v1 SimHash。
  服务端无原文不可重算历史指纹，v0/v1 并存平滑迁移：detect 的 exact 与
  变体判定按版本隔离（v1 指纹只与 `3` 开头模板比距离），旧模板不失效，
  新模板随新上报自然积累；服务端 / 快照 / 上报协议零变更
- 已知局限（corpus 记录）：中文换词变体（同义替换）的汉明距离普遍 >2，
  在 16 位 hex 协议内不做变体命中，仅 exact 覆盖；不提高全局阈值（误伤风险）
